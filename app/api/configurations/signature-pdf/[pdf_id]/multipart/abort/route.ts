...
export async function POST(
    ...
    try {
        ...
        const abortResponse = await fetch(
            `${apiBaseUrl}/api/v1/contractors/${contractorId}/configurations/signature-pdf/${pdf_id}/multipart/abort`,
            {
                method: 'POST',
                headers: {
        ...
        if (!abortResponse.ok) {
            ...
            return NextResponse.json(
                { message: errorData.detail || errorData.message || 'Failed to abort multipart upload' },
                { status: abortResponse.status }
            );
        }

        return NextResponse.json({ message: 'Multipart upload aborted successfully' });
    } catch (error) {
