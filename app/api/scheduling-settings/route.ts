    ...
    try {
        ...
        if (!res.ok) {
            return NextResponse.json(
                { message: data.detail || data.message || 'Failed to load scheduling settings' },
                { status: res.status }
            );
        }
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!res.ok) {
            return NextResponse.json(
                { message: data.detail || data.message || 'Failed to save scheduling settings' },
                { status: res.status }
            );
        }
        return NextResponse.json(data);
        ...
        // eslint-disable-next-line no-console
        console.error('scheduling-settings PUT', e);
        return NextResponse.json({ message: 'Server error' }, { status: 500 });
    }
}
